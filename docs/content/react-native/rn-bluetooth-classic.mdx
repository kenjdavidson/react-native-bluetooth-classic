...
Requests an array of the connected `BluetoothDevice`(s); in this context connected means an active `Socket`.
...
Requests whether a device `BluetoothDevice` is currently connected; in this context connected means an active `Socket`.
...
Provides a listener for events fired when `Socket` connections are: closed, dropped, failed, etc.  This was a hold over from the original library, it's a little more useful as a global check on connectivity than just using the `BluetoothDevice#disconnect()` function.
...
Attempt to change the name of the `BluetoothAdapter`.
