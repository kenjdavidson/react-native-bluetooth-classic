...
- Using the Korean charset [https://github.com/kenjdavidson/react-native-bluetooth-classic/issues/62](https://github.com/kenjdavidson/react-native-bluetooth-classic/issues/62)
...
There are a number of custom classes that are required, I'm not entirely happy with it as is, but it works and can be further absracted for a future release.

#### Implement `DeviceConnection` 

If the rest of the `DelimitedConnection*Impl` classes is acceptable, the easiest method for providing hex data is by extending the two classes and overriding the encode/decode methods.  There are a number of ways to do this, based on what and how you need things done:

- Build your client and accept `DeviceConnection` from scratch
- Extend the current `DelimitedConnection*Impl` classes and override the `encodeData` and `decodeData` methods

Choosing the latter would result in something like this:

```java
// com.example.bluetooth.conn.DelimitedHexConnectionClientImpl extends DelimitedConnectionClientImpl

private static final char[] HEX_ARRAY = "0123456789ABCDEF".toCharArray();

/**
 * Incoming data is encoded into HEX prior to writing it to the Buffer.
 */
@Override
...
public abstract class CustomBluetoothPackage {
  public static RNBluetoothClassicPackage PACKAGE = RNBluetoothClassicPackage.builder() 
    .withFactory('CLIENT', () -> new DelimitedHexConnectionClientImpl())
    .withFactory('SERVER', () -> new DelimitedHexConnectionAcceptImpl())
    .build();
}
```
